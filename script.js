...
/* v5.1 video hotfix
   Audio is the master playback source. The video is a synced visual layer,
   so toggling video should not restart, split, or replace the song. */
(function kingsMusicVideoHotfix(){
  function q(sel){ return document.querySelector(sel); }

  function getCurrentSongSafe(){
    try {
      if (typeof songs !== "undefined" && typeof currentIndex !== "undefined" && songs[currentIndex]) {
  ...
  function getEls(){
    return {
      audio: q("#audio"),
      video: q("#songVideo"),
      wideVideo: q("#wideVideo"),
      videoBox: q("#videoBox"),
      toggleVideo: q("#toggleVideo"),
      wideDialog: q("#wideVideoDialog"),
      videoWideBtn: q("#videoWideBtn")
    };
  }

  function isFileVideo(song){
    return song && song.video && (!song.videoType || song.videoType === "file" || song.video.endsWith(".mp4") || song.video.endsWith(".webm") || song.video.endsWith(".mov"));
  }

  function prepareVideoForCurrentSong(){
    const song = getCurrentSongSafe();
    const { audio, video, wideVideo, videoBox, toggleVideo } = getEls();
    if (!audio || !video || !videoBox || !toggleVideo) return;
    ...
    toggleVideo.hidden = false;

    if (video.getAttribute("src") !== song.video) {
      video.src = song.video;
      video.preload = "metadata";
      video.muted = true;          // prevents double-audio and allows easier synced playback
      video.playsInline = true;
      video.controls = true;
      video.load();
    }

    if (wideVideo && wideVideo.getAttribute("src") !== song.video) {
      wideVideo.src = song.video;
      wideVideo.preload = "metadata";
      wideVideo.muted = true;
      wideVideo.playsInline = true;
      wideVideo.controls = true;
      wideVideo.load();
    }
  }

  function syncVideoToAudio(force=false){
    const { audio, video } = getEls();
    const song = getCurrentSongSafe();
    if (!audio || !video || !isFileVideo(song) || videoBox?.hidden) return;

    if (Number.isFinite(audio.currentTime) && Number.isFinite(video.duration)) {
      const drift = Math.abs((video.currentTime || 0) - (audio.currentTime || 0));
      if (force || drift > 0.45) {
        try { video.currentTime = Math.min(audio.currentTime || 0, Math.max(video.duration - 0.2, 0)); }
        catch { try { video.currentTime = audio.currentTime || 0; } catch {} }
      }
    } else {
  ...
  // Override showVideo if the app defined it.
  window.showVideo = function(force){
    const { audio, video, videoBox } = getEls();
    const song = getCurrentSongSafe();
    if (!video || !videoBox || !isFileVideo(song)) return;

    prepareVideoForCurrentSong();

    const shouldShow = force === true ? true : force === false ? false : videoBox.hidden;
    videoBox.hidden = !shouldShow;
    ...
    if (shouldShow) {
      ...
      if (video.readyState >= 1) startSync();
  ...
  // Re-bind buttons after original script loads.
  window.addEventListener("load", () => {
    const { audio, video, wideVideo, toggleVideo, videoWideBtn, wideDialog } = getEls();

    if (toggleVideo) {
      toggleVideo.onclick = () => window.showVideo();
    }

    if (videoWideBtn && wideVideo && wideDialog) {
      videoWideBtn.onclick = () => {
        const song = getCurrentSongSafe();
        if (!isFileVideo(song)) return;
        if (wideVideo.getAttribute("src") !== song.video) {
          wideVideo.src = song.video;
          wideVideo.muted = true;
          wideVideo.load();
        }
        try { wideVideo.currentTime = audio?.currentTime || video?.currentTime || 0; } catch {}
        wideDialog.showModal();
        wideVideo.play().catch(()=>{});
      };
    }

    const closeWide = q("#closeWideVideo");
    if (closeWide && wideVideo && wideDialog) {
      closeWide.onclick = () => {
        if (audio && Number.isFinite(wideVideo.currentTime)) {
          audio.currentTime = wideVideo.currentTime;
        }
        wideVideo.pause();
        wideDialog.close();
        syncVideoToAudio(true);
    ...
    if (audio) {
      audio.addEventListener("play", () => syncVideoToAudio(true));
      audio.addEventListener("pause", () => {
        const { video, wideVideo } = getEls();
        if (video) video.pause();
        if (wideVideo) wideVideo.pause();
      });
      audio.addEventListener("seeking", () => syncVideoToAudio(true));
      audio.addEventListener("seeked", () => syncVideoToAudio(true));
      audio.addEventListener("timeupdate", () => syncVideoToAudio(false));
      audio.addEventListener("loadedmetadata", () => setTimeout(prepareVideoForCurrentSong, 100));
    }

    // Watch player title changes as a simple signal that the current song changed.
    const title = q("#barTitle");
    if (title && "MutationObserver" in window) {
      new MutationObserver(() => setTimeout(prepareVideoForCurrentSong, 60))
        .observe(title, { childList: true, characterData: true, subtree: true });
    }

    setTimeout(prepareVideoForCurrentSong, 250);
  });
})();
