...
function addShareButtonsSafe(root=document){
  root.querySelectorAll('.song-row').forEach(row => {
    ...
    const playBtn = actions.querySelector('[data-play], [data-plplay]');
    let songIndex = -1;
    if(playBtn?.dataset?.play !== undefined) songIndex = Number(playBtn.dataset.play);
    if(playBtn?.dataset?.plplay !== undefined) {
      const visibleTitle = row.querySelector('.song-title')?.textContent?.trim();
      songIndex = songs.findIndex(s => s.title === visibleTitle);
    }
    if(songIndex < 0 || !songs[songIndex]) return;
...
const kmOriginalBindForShare = typeof bind === 'function' ? bind : null;
if(kmOriginalBindForShare){
  bind = function(root=document){
    kmOriginalBindForShare(root);
    addShareButtonsSafe(root);
  };
}
setTimeout(()=>addShareButtonsSafe(document), 400);
